...
set plugin_name=HISE Convolution Reverb
set plugin_project_path=XmlPresetBackups/ConvolutionReverb.xml
set plugin_type=effect
...
if %buildAAX%==1 (
  ...
  %hise_path% clean 
  %hise_path% export_ci %plugin_project_path% -ipp -t:%plugin_type% -p:AAX -a:x86x64
...
%hise_path% clean
%hise_path% export_ci %plugin_project_path% -ipp -t:%plugin_type% -p:VST -a:x86x64
call Binaries/batchCompile.bat
